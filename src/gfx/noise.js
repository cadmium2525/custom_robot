/**
 * Noise kit for procedural texture synthesis.
 *
 * Deliberately CPU-side and seedable: textures are baked once at load into
 * canvases, so a phone pays the cost during the loading screen instead of every
 * frame. All functions are pure given a seed.
 */
...
export class Noise {
  ...
  reseed(seed) {
    ...
    // xorshift shuffle so the same seed always yields the same table.
    let x = seed >>> 0 || 1;
    for (let i = 255; i > 0; i--) {
      x ^= x << 13; x >>>= 0;
      x ^= x >>> 17;
  ...
  /** Ridged fbm — the sharp creases that read as machined metal. */
  ...
  /** Tiling 2D fbm via 4D-ish trick: sample on a torus using two 3D slices. */
  tileFbm2(x, y, period = 1, octaves = 5) {
...
export function worley2(x, y, seed = 0) {
  ...
  for (let dy = -1; dy <= 1; dy++) {
    for (let dx = -1; dx <= 1; dx++) {
      ...
      // Deterministic per-cell feature point.
      let h = (cx * 374761393 + cy * 668265263 + seed * 1274126177) | 0;
      h = (h ^ (h >>> 13)) * 1274126177 | 0;
      const rx = ((h >>> 8) & 0xffff) / 65535;
      const ry = ((h >>> 20) & 0xfff) / 4095;
